...
class VGG(nn.Module):
    def __init__(self, features, num_classes=10, init_weights=True, return_features=False, image_size=None):
    ...
    def forward(self, x):
        ...
        if self.return_features is False:
            x = self.features(x)
        else:
            # output from max pooling as features
            for module in self.features:
                x = module(x)
                if isinstance(module, nn.Conv2d):
                    all_features.append(x)
                elif isinstance(module, nn.MaxPool2d):
                    selected_feature_lst.append(all_features[-1])

            assert len(selected_feature_lst) > 0
